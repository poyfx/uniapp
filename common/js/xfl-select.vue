<template>
	<div class="show-box" :class="{disabled: disabled, active: isShowList}" :style="style_Container">
		<image src="../../static/img/statusIcon.png" mode="aspectFit" style="width: 15px; height: 16px; position: absolute;    top: 50%;
		margin-top: -8px;left: 20px; z-index: 1;"
		 @tap="upperClick"></image>
		<input v-if="showInput" class="input" type="text" v-model="selectText" :placeholder="placeholder" @focus="focus"
		 @blur="blur" @input="input" @confirm="$emit('confirm', $event)">
		<div v-else class="input" @tap="upperClick">
			{{selectText}}
			<view class="status_days_text">
				筛选
			</view>

		</div>

		<image src="../../static/img/select.png" mode="aspectFit" style="width: 10px; height: 10px; position: absolute;    top: 50%;
		margin-top: -4px;right: 5px; z-index: 1;"
		 @tap="upperClick"></image>
		<!-- <span 
			@click="toggleList" 
		...
		<div class="list-container" @tap.stop="listClick" :style="'top:' + boxHeight + 'px;'" v-show="isShowList">
			<!-- <span class="popper__arrow"></span> -->
			<scroll-view :class="showhids? shows:hides" class="list" style="background-color: #fff;" :style="'max-height: ' + showBoxHeight +'em;'"
			 scroll-y=true scroll-x=true>
				<div class="item" @tap="clickItem(index, item.value)" v-for="(item, index) in innerList" :key="index" :class="{active: activeIndex == index, disabled: item.disabled}">
					<div>{{item.value}}</div>
				</div>
...
<style scoped lang="less">
	...
	.show-box {
		...
		line-height: inherit;
		outline: none;
		padding: 10px @arrowWidth 10px @padding-left;
		...
		.list-container {
			...
			.shows {
				animation: 0.3s myshow;
			}

			.hides {
				animation: 0.3s myhide;
			}
